...
if c_jin_pengumpul>0:
    update_csv('bahan_bangunan.csv', 1, 2, int(data_bahan[1][2]) + pasir_terkumpul)
    update_csv('bahan_bangunan.csv', 2, 2, int(data_bahan[2][2]) + batu_terkumpul)
    update_csv('bahan_bangunan.csv', 3, 2, int(data_bahan[3][2]) + air_terkumpul)
